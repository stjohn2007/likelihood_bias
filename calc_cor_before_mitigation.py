...
def main():
    ...
    evaluator_dir = base_dir / "evaluator" / "baseline" / args.data_name / eval_name
    ...
    for aspect in aspect_names:
        ...
        evaluator_path = evaluator_dir / aspect / "test" / file_name
        ...
        unfairness_score = [eval_score - hum_score for eval_score, hum_score in zip(rescale_score(evaluator_score), rescale_score(human_score))]
        average_evaluator_score = [eval_score + tmp_eval_score for eval_score, tmp_eval_score in zip(evaluator_score, average_evaluator_score)]
        average_human_score = [hum_score + tmp_hum_score for hum_score, tmp_hum_score in zip(human_score, average_human_score)]
        line = [
    ...
    average_unfairness_score = [eval_score - hum_score for eval_score, hum_score in zip(rescale_score(average_evaluator_score), rescale_score(average_human_score))]
    line = [
    ...
    output_file = "cor.csv"
    output_path = Path("./mitigation_result") / "cor" / "before" / args.data_name / f"{eval_name}-{likelihood_name}" / output_file
